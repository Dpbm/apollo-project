...
By means of easing the process of understanding the data and effectively creating the classification model, we employed the following tools: \textit{Jupyter Lab} (for first analysis), \textit{Python}, \textit{Streamlit}, \textit{Scikit-Learn}, \textit{Imbalanced-Learn}, \textit{Pandas} and \textit{Numpy}.

At the end of the project, we created a pipeline extracting all the knowledge we gained from our \textit{a priori} analysis done in \textit{Jupyter Notebooks}. With this pipeline, we connected the parts in an easy-to-use dashboard with \textit{Streamlit} which is accessible via this link \href{https://apollo-project.streamlit.app/}{https://apollo-project.streamlit.app/}. Also, the code and experiments can be seen in this github repository \href{https://github.com/Dpbm/apollo-project}{https://github.com/Dpbm/apollo-project}, as well as in the zip file within the attachments.

In this document, we aim to show our thoughts, methodologies and insights we extracted during the process. In Section \ref{sec1}, we’re going to present how the data was parsed, transformed, analyzed and what we found at that step. In Section \ref{sec2}, we’re going to discuss the model creation, training and evaluation. In Section \ref{sec3}, we’re going to show the output metrics and analyze them. Finally, In Section \ref{sec4} we’re going to give overall insights and briefly discuss future works and improvements.

\section{Data Analysis} \label{sec1}

The provided data was a pickle file, so the first step was to open it and understand its internal structure. It was known that it was a python Dictionary with $3$ levels of data following the structure as shown below in Listing.\ref{structure} .
...
\begin{listing}[H]
	...
	\end{minted}
	\caption{The pickle structure.}
	\label{structure}
\end{listing}

However, we did not know exactly how the ids nor the embedding looked like. So first of all, we employed the \textit{python's built-in methods} for dictionaries \textit{.values()} and \textit{.keys()} for each \textit{JSON} level, aiming to get a grasp of how the syndromes, subjects and images were distributed. To ease the visualization and allow us to find inconsistencies within the data, we extract the object characteristics to a \textit{Pandas DataFrame}, this way we could manipulate the data as we wanted efficiently. For that, we used the code in Listing.\ref{extract}.
...
mapping.head()
	...
	\caption{\textit{Python} code for extracting dataset characteristics into a \textit{Pandas DataFrame}}
	\label{extract}
\end{listing}

Using this DataFrame, we could easily filter for \textit{Nan} values, embeddings without the correct amount of dimensions ($320$), understand how many images per syndrome or either per subject the dataset has, etc. Another interesting characteristic we extracted was the min and max values for each embedding. We did it aiming to see if there were any data that could be too far away in some dimension or either too close, giving us the ability to plot the distribution later.

Now for the embeddings, we had to convert the dictionary data into a more suitable format, in this case we moved the embeddings array into a \textit{Numpy ndarray}. This new structure allowed us to apply transformations and plot it as we wished. The final shape of this condensed block of embeddings was $(1116, 320)$, being in total $1116$ images/rows in total. During the process, we also checked for invalid data, like \textit{np.nan} and \textit{np.inf}, getting hid of them using \textit{np.nan\_to\_num}. The algorithm is in Listing.\ref{ndarray}.
...
for i,row in mapping.iterrows():
	...
	\end{minted}
	\caption{\textit{Python} the creation of a ndarray containing all images in sequence.}
	\label{ndarray}
\end{listing}

This size of it was measured and it consumed $\approx 1Mb$.
...
Using \textit{Pandas built-in methods}, we started checking for incongruent points of data. Running the \textit{.describe()} method, we were allowed to look for numerical incongruence within the data, but none was found as show at Table.\ref{table-df}.
...
\begin{table}[!h]
	\begin{center} 
		\begin{tabular}{ |c|c|c|c| } 
			...
			\hline
		\end{tabular}
		\caption{\textit{Pandas} \textit{.describe()} output for mapping DataFrame.}
		\label{table-df}
...
We also collected some primary information about the amount of syndromes, subjects and images shown in Listing.\ref{overall}.
...
\end{minted}
	...
	\caption{Overall statistics for number of syndromes, subjects and images in mapping DataFrame.}
	\label{overall}
\end{listing}

At this point, we started implementing the dashboard using \textit{Streamlit} and also extracted a few more statistics from it Listing.\ref{statistics}.
...
\end{minted}
	...
	\caption{\textit{Python} function to get overall statistics from the input DataFrame. Some more characteristics were extracted this time.}
	\label{statistics}
\end{listing}
\newpage
...
The plots on Figure.\ref{statistics-dashboard} show us that the input data was imbalanced across different labels. While some had $\approx 200$ images, others had $\approx 50$, being an alarming factor. To overcome this, we focused on applying \textit{under-sampling} techniques. For this case, we tested different approaches, but the one who best fit our needs was \textit{RepeatedEditedNearestNeighbours}, method available in the \textit{Imbalanced-Learn Python package}. This method uses the \textit{KNN(k-nearest neighbors)} algorithm to clean the noisy data, prevailing the clusters with well defined bounds. 

A better approach would be \textit{data augmentation}, but we do not know exactly how the data looked like previously, so it was not suitable for our case. 

An also interesting to notice fact present on the plot Figure.\ref{subjects-images}, which is also affirmed by Figure.\ref{imgs-per-subject}, is the pair subject-image. The plot behavior follows the expected rule that, with more people having a syndrome (subject), more images we'll have. So, those syndromes with less images might be considered rare. Due to this fact, it may be the reason for the imbalanced dataset. This fact is an overall problem (when not thinking about human health) for the model, once it may be too difficult to find this sort of data in the wild at all, since the occurrences are low.

\subsection{T-SNE}

Since we had a $320$-dimensional embedding, it is not possible to plot it right away, so we needed a different approach. 

The \textit{T-SNE} is an algorithm designed to reduce the dimensionality of data, as done by the well-known algorithm \textit{PCA}, but preserving its local structure.

We applied this algorithm throughout our experiments, as a way to visualize the clusters. We also applied it to test the under-sampling techniques as shown at Figure.\ref{tsne-plots}.
...
For the model, we used \textit{KNN (k-nearest neighbors)} algorithm, being one of the most common algorithms for classifying points by their proximity.

In this experiment, we tested two different metrics for \textit{KNN}, being them \textit{cosine} and \textit{euclidean}. Also, different parameters of \textit{n-neighbors} were experimented, ranging from $1$ to $15$. For this task, we used \textit{GridSearchCV} from \textit{Scikit-Learn}, to apply different combinations of these parameters and give a rank with the bests. The search used accuracy as a ranking metric and the training for each parameter was split across $10$ folds for Cross-Validation.

The outcomes pointed $k=1$ for \textit{cosine} and $k=6$ for \textit{euclidean}, which are lower than expected but it is due to the dataset reduction after under-sampling. In total we had $2$ models to use for predicting values.

The data used was the resampled dataset we created in Section.\ref{sec1}, split into train and test parted $80-20\%$ respectively. Then, the training batch was separated into $k=10$ folds for Cross-Validation and the remaining test portion was used later for evaluation.
...
To ensure our model was generalizing and evolving, we used different metrics and calculated that for each fold and after evaluation. The metrics used were: accuracy, recall (micro), precision (micro), f1 (per label), ROC-AUC (OVR) and Top-K. In addition, we calculated a confusion matrix for each fold and evaluation.

For ROC-AUC per fold, we calculated and averaged that at the end, so we got, indeed, the average ROC-AUC per label across all $k$ folds. The method used for that is discussed in References \cite{_receiver} and \cite{shen_2017_how}.

The results are shown in Figure.[\ref{results-per-fold}, \ref{results-overall}] and Table.\ref{table-eval}.
...
The results are decent given the amount of images used for training. The models reached a good precision and their ROC-AUC curve are high.

Comparing both models, Euclidean was the best, which might be due to our transformed data that had a better proximity and a straight line was the best to find neighbors than comparing angles for cosine.

One point that was observed, is that the AUC was lower for sparse classes and classes with lower density, which is understandable since the model had less chance to understand these components as clusters. 

Finally, is also notable that the final performance kept faithful to what was seen during training, being always $\geq{0.8}$ for every metric.
...
During the development, we found that the data itself was imbalanced, requiring some sort of manipulating and under-sampling. After our analysis, it stacks out that it is likely that this occurred because the lack of some diseases data, especially those that are rare. Even though they are real and occur, the model may get confused and it results in worse performance. The best option would be generating synthetic data, so it depends deeply on how the original data was alike.

Also, the final results pointed that sparse data (like the syndrome \textit{700018215}) and smaller clusters (like the syndrome \textit{100610883}) can lead to lower performance. Saying so, an improvement point for this dataset is increasing its diversity.

For future works we would like to state some ideas to work with. First, using a different model for comparison, like \textit{SVM} or even creating a \textit{Deep Neural Network}. Also, the \textit{T-SNE} is sometimes hard to understand, since it doesn't preserve the global relationship but only local, so using \textit{UMAP} may be a better option and a got starting point for next experiments.
